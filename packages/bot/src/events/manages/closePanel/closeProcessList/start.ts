...
import { makeEditCloseProcessList } from '../../../../settingPanel';
...
export async function execute(interaction: ButtonInteraction): Promise<void> {
	...
	if (interaction.customId !== `edit_closeProcess`) return;
...
const main = async (interaction: ButtonInteraction) => {
	...
	await makeEditCloseProcessList(panelId, interaction.channel);
};
