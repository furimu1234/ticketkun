...
export async function execute(interaction: ButtonInteraction): Promise<void> {
	...
	if (action !== 'ignoreMembers') return;
...
const main = async (
	...
	if (process.name !== 'ignoreMembers') return;
	...
	while (true) {
		...
		if (reply.customId === 'edit_ignore_roles') {
			const select = selector(interChannel, '除外ロールを選択してください!');
			select.setMinSize(0);
		...
		if (reply.customId === 'edit_ignore_users') {
			const select = selector(interChannel, '除外ユーザを選択してください!');
			select.setMinSize(0);
			select.setMaxSize(25);
...
const makeContainer = (process: CloseProcessStep) => {
	...
	containerBuilder.addSectionComponents(
		addSectionWithButtonBuilder({
			...
			buttonLabel: '除外ロール変更',
			buttonCustomId: 'edit_ignore_roles',
	...
	containerBuilder.addSectionComponents(
		addSectionWithButtonBuilder({
			...
			buttonLabel: '除外ユーザ変更',
			buttonCustomId: 'edit_ignore_users',
			buttonStyle: ButtonStyle.Danger,
		}),
	);
