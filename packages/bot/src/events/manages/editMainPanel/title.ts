...
const main = async (interaction: ButtonInteraction) => {
	...
	const model = await store.do(async (db) => {
		...
		const label = new LabelBuilder()
			...
			.setTextInputComponent(
				new TextInputBuilder()
					...
					.setValue(model.mainPanel.embeds[0].title ?? '')
		...
		model.mainPanel.embeds[0].title = modalR.getTextInputValue(newItemCustomId);

		checkPanelName(model.mainPanel.content, model.mainPanel.embeds);
