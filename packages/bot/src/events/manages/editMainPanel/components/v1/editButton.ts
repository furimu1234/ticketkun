...
export async function execute(interaction: ButtonInteraction): Promise<void> {
	...
	if (!interaction.customId.startsWith(`editMainPanelComponent-`)) return;
...
const main = async (interaction: ButtonInteraction) => {
	...
	await makeEditMainPanelButton(targetCustomId, targetComponent, panelId);
};
...
export const getTargetButtonComponent = async (
	...
	const rowsModel = model.mainPanel.rows;
